...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        {
            dataBuffer.at(0) = dataBuffer.at(1);
            dataBuffer.at(1) = frame;
        }
        ...
        if (bFocusOnVehicle)
        {
          vector<cv::KeyPoint> keypoint_rect;
          for(auto i=keypoints.begin(); i!=keypoints.end(); i++)
          {
              if(vehicleRect.contains(i->pt))
              {
                cv::KeyPoint roi_kp;
                roi_kp.pt = cv::Point2f(i->pt);
                roi_kp.size = 1;
                keypoint_rect.push_back(roi_kp);             
              }
          } 
          keypoints = keypoint_rect;
        }
